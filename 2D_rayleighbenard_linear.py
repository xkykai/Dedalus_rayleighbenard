...
problem.add_equation("dt(b) - kappa*div(grad_b) + lift(tau_b2) = 0")
problem.add_equation("dt(u) - nu*div(grad_u) - f*v + grad_p@ex + lift(tau_u2) = 0")
problem.add_equation("dt(v) - nu*div(grad_v) + f*u + lift(tau_v2) = 0")
problem.add_equation("dt(w) - nu*div(grad_w) + grad_p@ez - b + lift(tau_w2) = 0")
...
snapshots = solver.evaluator.add_file_handler(f"Data/linear_uvwb_noise_snapshots_Nz_{Nz}_Ra_{Ra_str}_Ta_{Ta_str}", sim_dt=save_dt, max_writes=50)
