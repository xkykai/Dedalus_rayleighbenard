...
if __name__ == "__main__":
    ...
    frames = sorted(glob.glob(str(output_path.joinpath('write_*.png'))))

    fig, ax = plt.subplots()
    ims = []
    for frame in frames:
        im = plt.imread(frame)
        ims.append([ax.imshow(im, animated=True)])

    ani = animation.ArtistAnimation(fig, ims, interval=33, blit=True)
    ani.save(str(output_path.joinpath('animation.mp4')), writer='ffmpeg', fps=30, dpi=300)
